...
public:
    ...
    static void displayBooks(const addebook books[], int book_count) {
        if (book_count == 0) {
...
public:
    ...
    static void borrowBook(borrow borrows[], int &borrow_count) {
        if (borrow_count < MAX_BORROWS) {
            ...
            cin >> u_phone;
            borrows[borrow_count] = borrow(u_id, u_name, b_name, a_name, u_email, u_phone);
            borrow_count++;
    ...
    static void displayBorrows(const borrow borrows[], int borrow_count) {
        if (borrow_count == 0) {
...
int main() {
    ...
    int book_count = 0, borrow_count = 0; // To keep track of number of books and borrow records
    int choice;
    ...
    while (true) {
        ...
        switch (choice) {
            case 1:
                ...
                break;
            case 2:
                addebook::displayBooks(books, book_count);
                break;
            case 3:
                borrow::borrowBook(borrows, borrow_count);
                break;
            case 4:
                borrow::displayBorrows(borrows, borrow_count);
                break;
            case 5:
                cout << "Thank You!\n";
                return 0;
